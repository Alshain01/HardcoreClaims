...
public class HardcoreClaims extends JavaPlugin {
		...
		private void onCommandPreprocess(PlayerCommandPreprocessEvent e) {
			...
			if (e.getMessage().toLowerCase().contains("abandonclaim")
					|| e.getMessage().toLowerCase()
							.contains("abandontoplevelclaim")) {
				e.getPlayer()
						.sendMessage(ChatColor.RED + "You may not abandon claims in hardcore worlds.");
				e.setCancelled(true);
			}
		}
	...
	private class ContainerGuard implements Listener {
		...
		private void onBlockPlace(BlockPlaceEvent e) {
			...
			case CHEST:
				...
				if (GriefPrevention.instance.dataStore
						.getPlayerData(e.getPlayer().getName()).claims.size() == 0) {
					break;
				}
			case ENDER_CHEST:
			case TRAPPED_CHEST:
			...
			case STORAGE_MINECART:
				final Claim claim = GriefPrevention.instance.dataStore
						.getClaimAt(e.getBlock().getLocation(), true, null);
				if (claim == null
						|| !claim.getOwnerName()
								.equals(e.getPlayer().getName())) {
					e.getPlayer()
						.sendMessage(ChatColor.RED + "You may not place containers outside your claim in hardcore worlds.");
					e.setCancelled(true);
				}
				break;
			default:
	...
	private class Reaper implements Listener {
		...
		private void onPlayerDeath(PlayerDeathEvent e) {
			...
			for (final Claim c : GriefPrevention.instance.dataStore
					.getPlayerData(e.getEntity().getName()).claims) {
				// Does the claim belong to the corpse
				if (c == null || !c.inDataStore
				...
				if (hcFlag != null
						&& !new GriefPreventionClaim(
								c.getGreaterBoundaryCorner()).getValue(
								(Flag) hcFlag, false)) {
					continue;
				}
		...
		private void removeTamedAnimals(Player player) {
			for (final World w : Bukkit.getWorlds()) {
				...
				for (final Entity e : w.getEntities()) {
					...
					final Tameable t = (Tameable) e;
					if (!t.isTamed()
							|| !t.getOwner().getName().equals(player.getName())) {
						continue;
					}
	...
	public void onEnable() {
		...
		if (!getServer().getPluginManager().isPluginEnabled("GriefPrevention")) {
			getLogger()
					.info("Grief Prevention is not installed. HardcoreClaims is shutting down");
